<template>
  <div style="padding: 2px 0 2px 10px">
    <div class="iqis-upload-list" v-for="(item,index) in myUploadList" :key="index" :style="{width:($props.size)+'px',height:($props.size)+'px'}">
    ...
    <Modal :title="imgName" v-model="visible">
      <img :src="imgUrl" v-if="visible&&!file" style="width: 100%">
      <a v-if="file" :href="imgUrl.endsWith('.pdf')?this.$http.defaults.baseURL+'/uploads'+imgUrl:'http://view.officeapps.live.com/op/view.aspx?src='+imgUrl" target="_blank" @click="visible=false">{{$t('iqisUpload.onlineView')}}</a>
      <div slot="footer">
  ...
  export default {
    ...
    methods: {
      ...
      handleRemove (file,event) {
        ...
        if(this.isProdPage){
          this.$refs.upload.fileList[del] = null
          
        }else{
          this.$refs.upload.fileList.splice(fileList.indexOf(file), 1);
        }
        this.myUploadList=this.$refs.upload.fileList
        console.log(this.myUploadList,'myUploadList');
      ...
      handleSuccess (res, file) {
        ...
        that.imgSrc(i)
        that.$emit('getPictureUrls',that.myUploadList)
      },
