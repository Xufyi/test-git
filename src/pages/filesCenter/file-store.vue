  ...
  <div class="iqis-page">
    ...
    <div class="iqis-page-tool-row">
      <div class="iqis-page-tool-primary" style="font-size:0;">
        <Button type="ghost" class='initButtons'  v-if='uploadPermissions'  icon="upload" @click="addUpload()" >{{$t('fileCenter.upload')}}</Button>
        <Button style="margin:0 8px;" type="ghost" class='initButtons'  v-if='addBagPermissions' icon="addwjj"  @click="addFileBag()">{{$t('fileCenter.addFolder')}}</Button>
        <Button type="ghost"  class='initButtons'  v-if='downloadPermissions' icon="download" :disabled="tableEditDis" @click='download'>{{$t('fileCenter.download')}}</Button>
        <Button style="margin:0 8px;" type="ghost"  class='initButtons'  v-if='movePermissions' icon="move" :disabled="tableEditDis" @click="moveFile">{{$t('fileCenter.move')}}</Button>
        <Button type="ghost"  class='initButtons'  v-if='QXPermissions' icon="edit" :disabled="tableEditDis" @click='quanxian()'>{{$t('fileCenter.permission')}}</Button>
        <Poptip  :ok-text="i18n.t('confirm')" :cancel-text="i18n.t('cancel')"
          confirm
          :title="i18n.t('fileCenter.dsi')"
          placement="bottom-start"
          @on-ok="deleteTableSelected">
          <Button style="margin-left:8px;" type="ghost"  icon="trash-a" v-if='deletePermissions' :disabled="!table.selected">{{$t('fileCenter.delete')}}</Button>
        </Poptip>
      </div>
      <div class="iqis-page-tool-second">
        <Button type="text"  icon="refresh" @click="refreshData(parentId)">{{$t('fileCenter.refreshData')}}</Button>
      </div>
    </div>
    ...
    <div class="iqis-page-context">
     ...
     <Modal
      ...
      <Row type="flex" :gutter="40">
        ...
        <Row type="flex" :gutter="40">
          <i-col span="24">
            <Input  :placeholder="i18n.t('fileCenter.pe')" v-model="addFileBagName" style="width: 100%;"></Input>
          </i-col>
        </Row>
        ...
        <Row type="flex" :gutter="40">
          <i-col span="24">
            <Input  :placeholder="i18n.t('fileCenter.pe')" v-model="remarks" type='textarea' style="width: 100%;" :maxlength="Number(600)"></Input>
          </i-col>
        </Row>
        <div slot="footer">
      ...
      >
        ...
        <div class="filesList">
          <ul>
            <li class='tabTop fileItem' v-for="(item,index) in uploadLists">
              <div class='fileName'><img :src="'./static/fileIcon/'+item.type+'.svg'" class='initFileImg'  alt=""> {{item.name}} <span>( {{item.sizes}} )</span>  </div>
        ...
        <div class="userList">
          <CheckboxGroup v-model="QX">
            <Checkbox v-for="(item,index) in userList" class='initCheckBox' :label="item.jobId" :key="item.jobId">{{item.jobName}}</Checkbox>
          </CheckboxGroup>
        </div>
        ...
        <Row type="flex" :gutter="40">
          <i-col span="24">
            <Input  :placeholder="i18n.t('fileCenter.pe')" v-model="remarks" type='textarea' style="width: 100%;"></Input>
          </i-col>
        </Row>
        <div slot="footer">
      ...
      :mask-closable="false">
        ...
        <Row type="flex" :gutter="40" style='margin-bottom:16px'>
          <i-col span="12">
            <div class='infoItem'>
              <span class='left'>{{$t('fileCenter.fileType')}}：</span><span class='right'>{{fileInfoData.isDirectory === 0?'文件夹':fileInfoData.type}}</span>
            </div>
          </i-col>
          <i-col span="12">
            <div class='infoItem'>
...
<script>
  ...
  export default {
    ...
    data: function () {
      ...
      addBagPermissions:false,
      downloadPermissions:false,
      movePermissions:false,
      QXPermissions:false,
      deletePermissions:false,
      updateRemarkPermissions:false,
      updateNamePermissions:false,
      reg:/[<|>|\|\/\/|?|:|*|"|\/|]/,
      onUploading:false,
      fileInfoData:{
        ...
        table:{
          ...
          columns:[
            ...
            },
            {
                ...
                [
                  ...
                  h('input',{
                    ...
                    domProps:{
                      ...
                      className:'INITinput',
                      id:'filesInputID'+params.row.type+params.index
                    },
                    on:{
                      blur:(e)=>{
                        let type = params.row.type?'.'+params.row.type:'';
                        if(params.row.isDirectory === 0 && this.reg.test(e.target.value)){
                          this.$Notice.error({
                            title:i18n.t('fileCenter.renameError'),
                            desc:"i18n.t('fileCenter.folderNoAppear')"+' < > \ / ? : " * / '
                          })
                          document.getElementById('filesInputID'+params.row.type+params.index).focus();
                          return
                        }
                        ...
                        if((e.target.value+type) !== params.row.name){
                          ...
                          // this.data[params.index].changeState = true;
                        }
                        this.$nextTick(()=>{
              ...
              key: 'version',
              render:(h,params)=>{
                return h('span',params.row.version?params.row.version:'-');
              }
            },
            {
                ...
                return h('div', [
                    h('Button', {
                        ...
                        style: {
                          color:'#418FEE',
                          marginRight: '12px',
                          display:this.updateNamePermissions ? 'inline-block':'none'
                        },
                        on: {
                            click: () => {
                                this.data[params.index].changeState = true;
                    ...
                    h('Button', {
                        ...
                        style:{
                          color:'#418FEE',
                          display:params.row.isDirectory === 1 && this.QXPermissions ? 'inline-block' : 'none'
                        },
                        on: {
                            click: () => {
                                this.clickQX(params.index)
                            }
                        }
                    }, i18n.t('fileCenter.permission'))
                ]
                );
            }
            },{
              ...
              render:(h,params)=>{
                return h('div',[
                  h('span',{
                    ...
                    on:{
                      click:() => {
                        if(!this.updateRemarkPermissions){
                          return;
                        }
                        ...
                        this.$nextTick(()=>{
                          ...
                          // //console.log(document.getElementById('RemarkID'+params.row.name+params.index).children[0])
                        })
                      }
                    }
                  ...
                  h('Input',{
                    ...
                    on:{
                      'on-blur':(e)=>{
                        ...
                        // //console.log(e)
                        // return;
                        this.$http.put('/basic/remarks/file',{fileId:params.row.fileId,remarks:e.target.value}).then(res =>{
                          this.data[params.index].remarks = e.target.value;
                        });
                      }
                    }
                  })
    ...
    created(){
      ...
      this.$nextTick(function(){
        ...
        var noData='<img style="position:relative;left:-13px;" src="static/img/list-empty.png" :alt="i18n.t(\'fileCenter.noData\')"><div style="font-size:18px;line-height:24px;margin-top:27px;">'+i18n.t('fileCenter.noDataAddData')+'</div>'
    ...
    methods:{
      ...
      download(){
        ...
        for(let i=0;i<list.length;i++){
            if(list[i].isDirectory === 0){
              this.$Notice.warning({
                title: i18n.t('fileCenter.notSelectFolder'),
              });
              break;
              return;
            }
            this.$iqis.download('/basic/download/file',{fileId:list[i].fileId},'get',()=>{});
        }
      },
      moveFileOK(){
        let treeData = [];
      ...
      uploadSQL(){
        ...
        for(let i=0;i<this.uploadLists.length;i++){
          ...
          if(item.version === ''){
            state = true;
            this.$Notice.error({
              title:i18n.t('fileCenter.qtx')+(i+1)+i18n.t('fileCenter.fileVersion'),
              desc:i18n.t('fileCenter.versionNull')
            })
            break;
          }
          let itemFile = {
        ...
        if(params.length > 0 && !state){
          for(let i=0;i<params.length;i++){
            this.$http.post('/basic/file',params[i],this.headers).then(res=>{
           ...
           //console.log(file,'asd')
            ...
            form.append('filename',file.name);
                ...
                xhr.onload = function (res) {
                  if(that.onUploading){
                    setTimeout(function(){
                          if( that.uploadLists[that.uploadLists.length-1].uploadingNum < 100){
                            that.$Notice.error({
                              ...
                              desc:i18n.t('fileCenter.uploadFail')
                            });
                    ...
                    if (xhr.status === 201) {
                      ...
                      that.$Notice.error({
                        title: i18n.t('fileCenter.uploadFail3'),
                        desc:i18n.t('fileCenter.fileTooLarge')
                      });
                      that.onUploading = false;
                ...
                xhr.upload.onprogress = function (event) {
                    if (event.lengthComputable && that.onUploading) {
      ...
      selectFilesCh(){
        if(!this.onUploading){
            ...
            for (let i=0;i<fileList.length;i++) {
              for(let j=0;j<this.uploadLists.length;j++){
                if(fileList[i].name === this.uploadLists[j].name){
                  this.$Notice.error({
                    title:i18n.t('fileCenter.fileRepeatAdd'),
                    desc:i18n.t('fileCenter.newFileAndAdd')+(j+1)+i18n.t('fileCenter.aFileRepeat')
                  })
      ...
      deleteFileList(k){
        if(this.uploadLists[k].uploading){
          this.onUploading = false;
        }
      ...
      selectAll(){
        ...
        if(this.checkAll1){
          for(let i=0;i<this.userList.length;i++){
            this.QX.push(this.userList[i].jobId);
          }
        }else{
            this.QX = [];
        }
      ...
      that.$http.get(url).then(res => {
        ...
        for(let i= 0;i<res.data.permissions.length;i++){
          let qx = res.data.permissions[i]
          if(qx === 'INSERT_FILE'){
            this.uploadPermissions = true;
          }
          if(qx === 'INSERT_DIRECTORY'){
            this.addBagPermissions = true;
          }
          if(qx === 'DOWNLOAD_FILE'){
            this.downloadPermissions = true;
          }
          if(qx === 'DELETE_FILE'){
            this.deletePermissions = true;
          }
          if(qx === 'UPDATE_FILE_PERMISSION'){
            this.QXPermissions = true;
          }
          if(qx === 'UPDATE_MOVE_FILE'){
            this.movePermissions = true;
          }
          if(qx === 'UPDATE_FILE_REMARK'){
            this.updateRemarkPermissions = true;
          }
          if(qx === 'UPDATE_RENAME_FILE'){
            this.updateNamePermissions = true;
          }
        }
      })
    },
        ...
        for(let i=0;i<list.length;i++){
          this.$http.delete('/basic/delete/file?fileIds[]='+list[i].fileId,this.headers).then(res=>{
            if(i === list.length-1){
              this.refreshData(this.parentId);
              this.$Notice.success({
                title: i18n.t('fileCenter.deleteSuccess'),
              });
              this.addFileBagsCan();
            }
          }).catch(err=>{
      ...
      spectorName() {
      let that = this;
      let url = "/systemManage/allJob?page=1&limit=99999";
      that.$http.get(url).then((res) => {
        let index = -1;
        for(let i=0;i<res.data.length;i++){
          if(res.data[i].jobName === i18n.t('fileCenter.administrators')){
            index = i;
            break;
          }
        }
      ...
      refreshData(parentId){
        ...
        let url = ''
        this.$http.get("/basic/file/"+this.parentId).then(function (res) {
          for(let i=0;i<res.data.length;i++){
      ...
      searchData(){
        ...
        this.$http.get("/basic/search/file",{params}).then(function (res) {
          for(let i=0;i<res.data.length;i++){
            res.data[i]._checked = false;
            res.data[i].changeState = false;
            res.data[i].changeRemarkState = false;
