  ...
  <div class="iqis-page">
    <div class="iqis-page-search-row">
      <div class="iqis-page-search-bar-item" style="flex:0 0 220px;">
        ...
        <!-- <DatePicker type="daterange" format="yyyy-MM-dd HH:mm"  v-model="search.dates" placement="bottom-start" placeholder="操作日期过滤" style="width: 200px"></DatePicker> -->
      </div>
    ...
    <div class="iqis-page-tool-row">
      <div class="iqis-page-tool-primary" style="font-size:0;">
        ...
        <Button type="ghost" icon="plus" @click="addIqc" v-if="permsBtn.INSERT_SUPPLIER_REVIEW">{{$t('iqc.add')}}</Button>
        <Button style="margin:0 8px;" type="ghost" v-if="permsBtn.UPDATE_SUPPLIER_REVIEW" icon="edit" :disabled="tableEditDis" @click="editSelectRow">{{$t('iqc.edit')}}</Button>
        <Poptip  :ok-text="i18n.t('confirm')" :cancel-text="i18n.t('cancel')" 
          confirm
          :title="i18n.t('iqc.confirmDelete')"
          placement="bottom-start"
          @on-ok="deleteTableSelected">
          <Button type="ghost" v-if="permsBtn.DELETE_SUPPLIER_REVIEW"  icon="trash-a" :disabled="!table.selected">{{$t('iqc.delete')}}</Button>
        </Poptip>
      </div>
    ...
    <div class="arrangement-dialog-from">
      <Modal
      :title="i18n.t('iqc.supplierReview')"
          ...
          <i-col span="12">
           <div class="fl nameWidth marginLF"><span>{{$t('iqc.supplierAbbr')}}</span></div>
            <div class="marginLF">
                ...
                <Row type="flex" :gutter="24" style="margin-bottom:15px">
          <i-col span="12">
            <div class="fl nameWidth marginLF"><span>{{$t('iqc.reviewMethod')}}</span></div>
            <div class="marginLF">
               <Select style="width: 100%" :placeholder="i18n.t('pleaseChoose')" :disabled="noPermsDis" v-model="ruleForm.hierarchyType" filterable class="Wout" >
                <Option v-for="(items,index) in ruleReview" :value="items.value" :label="items.label"  :key="index">{{ items.label}}</Option>
                </Select>
             </div>
          </i-col>
          <i-col span="12">
           <div class="fl nameWidth marginLF"><span>{{$t('iqc.reviewScore')}}</span></div>
            <div class="marginLF">
                <!-- <i-input v-model.trim="ruleForm.reviewScore" style="width: 100%" ></i-input> -->
                <InputNumber :disabled="noPermsDis" v-model="ruleForm.reviewScore" style="width: 100%" :min="0" :max="100"></InputNumber>
             </div>
          </i-col>
        </Row>
                <Row type="flex" :gutter="24" style="margin-bottom:15px">
          <i-col span="12">
            <div class="fl nameWidth" style="padding-right: 5px; width:100%;"><span>{{$t('iqc.planReviewTime')}}</span></div>
            <div class="marginLF">
                <Date-picker style="width: 100%" :disabled="noPermsDis" type="datetime" format="yyyy-MM-dd HH:mm" v-model="ruleForm.planReviewDate" ></Date-picker>
             </div>
          </i-col>
          <i-col span="12">
           <div class="fl nameWidth" style="padding-right: 5px; width:100%;"><span>{{$t('iqc.actualReviewTime')}}</span></div>
            <div class="marginLF">
                <Date-picker style="width: 100%" :disabled="noPermsDis" type="datetime" format="yyyy-MM-dd HH:mm" v-model="ruleForm.reviewDate" ></Date-picker>
             </div>
          </i-col>
        </Row>
        <Row type="flex" :gutter="24" style="margin-bottom:15px">
          ...
          <i-col span="24">
            ...
            <div >
                <div class="fl"  style="margin-right:30px">
                    <RadioGroup v-model="ruleForm.pushFlag">
  ...
  export default {
    ...
    data: function () {
      ...
      return {
        ...
        ruleReview:[
          {
          value: 'REVIEW',
          label: i18n.t('iqc.fieldReview')
          },{
          value: 'WRITTEN',
          label: i18n.t('iqc.writtenReview')
          }
        ],
        table:{
          selected:false,
          ...
          columns:[
            ...
            // },
            {
              ...
              title: i18n.t('iqc.planReviewTime'),
              className:'tablePadding',
              ...
              title: i18n.t('iqc.reviewMethod'),
              className:'tablePadding',
              minWidth:80,
              ellipsis:true,
              key: 'hierarchyType',
              render:vm.$iqis.utils.tableHelper.buildMapCell(vm,{'REVIEW':i18n.t('iqc.fieldReview'),'WRITTEN':i18n.t('iqc.writtenReview')})
            },{
              title: i18n.t('iqc.actualReviewTime'),
              className:'tablePadding',
              width:this.$store.state.locale=='zh_CN'?106:125,
              minWidth:this.$store.state.locale=='zh_CN'?106:125,
              ellipsis:true,
              key: 'reviewTime',
              render:function(h,data){
                let value = data.row[data.column.key];
              ...
              title: i18n.t('iqc.reviewScore'),
              className:'tablePadding',
              minWidth:70,
              ellipsis:true,
              key: 'reviewScore'
            },{
              title: i18n.t('iqc.reviewGrade'),
              className:'tablePadding',
              minWidth:70,
              ellipsis:true,
              key: 'reviewLevel'
            },{
              title: i18n.t('iqc.reviewer'),
              className:'tablePadding',
              width:110,
              minWidth:110,
              ellipsis:true,
        ...
        permsCur:[
          "DELETE_SUPPLIER_REVIEW",
          "INSERT_SUPPLIER_REVIEW",
          "UPDATE_SUPPLIER_REVIEW",
          "SELECT_SUPPLIER_REVIEW",
        ],//当前页面需要的权限名称
        permsBtn:{//控制每个权限对应按钮的禁用状态
          "DELETE_SUPPLIER_REVIEW":false,
          "INSERT_SUPPLIER_REVIEW":false,
          "UPDATE_SUPPLIER_REVIEW":false,
          "SELECT_SUPPLIER_REVIEW":false,
        },
        noPermsDis:true,
      }
    },//end data
    ...
    methods:{
      ...
      addIqc(){
        ...
        that.ruleForm.hierarchyType='';
        that.ruleForm.reviewScore=0;
        that.ruleForm.planReviewDate='';
        that.ruleForm.reviewDate='';
        that.onPopUp();
      ...
      editSelectRow(e){
        ...
        that.onPopUp();
        that.dataTable();
        let reviewId=selectedRow[0].reviewId
        let url="/iqc/manufacturer/review/"
        that.$http.get(url+reviewId).then((res)=>{
          that.ruleForm={
          reviewId:reviewId,
          manufacturerId:res.data.manufacturerId,
          hierarchyType:res.data.hierarchyType,
          reviewScore:res.data.reviewScore,
          planReviewDate:new Date(res.data.planReviewTime),
          reviewDate:new Date(res.data.reviewTime),
      ...
      onPopUp(){
        if((this.prodBatch.btnState=='post'&&this.permsBtn.INSERT_SUPPLIER_REVIEW)||(this.prodBatch.btnState=='put'&&this.permsBtn.UPDATE_SUPPLIER_REVIEW)){
      ...
      onDBClick(obj,index){
        ...
        that.uploadValue=res.data.manufacturerReviewExtInfoDtos
        that.spinShow =  true;
         that.prodBatch.state=true;
        })
      },
      tableSelectChange(selection){
        ...
        if(ss && ss.length > 0){
          ...
          for(let i=0;i<ss.length;i++){
            delArr.push(this.$http.delete("/iqc/manufacturer/review/"+ss[i].reviewId));
          }
          if(delArr.length > 0){
      ...
      refreshData(){
        ...
        this.$http.get("/iqc/manufacturer/review",{params:params}).then(function (resp) {
          that.data = resp.data;
     ...
     //编辑数据
      FormOk(){
       let that=this;
        ...
        that.ruleForm.manufacturerReviewExtInfoDtos=d3Arr
       let state = true;
       let params = [
        {type:'Empty',title:i18n.t('iqc.supplierNo'),value:that.ruleForm.manufacturerId},
        {type:'Empty',title:i18n.t('iqc.reviewMethod'),value:that.ruleForm.hierarchyType},
        {type:'Empty',title:i18n.t('iqc.reviewScore'),value:that.ruleForm.reviewScore},
        {type:'Empty',title:i18n.t('iqc.planReviewTime'),value:that.ruleForm.planReviewDate},
        {type:'Empty',title:i18n.t('iqc.actualReviewTime'),value:that.ruleForm.reviewDate},
      ]
      ...
      //     let hierarchyType=that.ruleForm.hierarchyType;
      //     let reviewScore=that.ruleForm.reviewScore;
      //     let planReviewDate=that.ruleForm.planReviewDate;
      //     let reviewDate=that.ruleForm.reviewDate;
      //     if(manufacturerId==''||hierarchyType==''||reviewScore==''||planReviewDate==''||reviewDate==''||manufacturerId==undefined||hierarchyType==undefined||reviewScore==undefined||planReviewDate==undefined||reviewDate==undefined){
      //          this.$Notice.warning({
      //         title: '请完善评审信息',desc: ''});
      //         return;
      //      }else{
      ...
      //      }
      if(index==true){
       that.ruleForm.planReviewTime = (new Date(that.ruleForm.planReviewDate)).getTime();
       that.ruleForm.reviewTime = (new Date(that.ruleForm.reviewDate)).getTime();
       if(that.ruleForm.pushFlag == 1){
