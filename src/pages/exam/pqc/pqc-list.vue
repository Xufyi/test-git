  ...
  <div class="iqis-page">
    <div class="iqis-page-search-row">
      ...
      <div class="iqis-page-search-bar-item lineheight34" style="flex:0 0 108px;">
        <Select v-model="search.result" :placeholder="i18n.t('pqc.tr')" clearable style="width: 100px;">
          <Option v-for="st in search.resultList" :key="st.value" :value="st.value">{{st.label}}</Option>
        </Select>
      </div>
      <div class="iqis-page-search-bar-item lineheight34" style="flex:0 0 108px;">
    ...
    <div class="iqis-page-tool-row">
      <div class="iqis-page-tool-primary" style="font-size:0;">
        ...
        <Button type="ghost" v-if="permsBtn.INSERT_PQC_ABNORMAL" @click="createAbnormal" :disabled="tableEditDis" icon="android-alert">{{$t("pqc.abnormality")}}</Button>
  ...
  export default {
        ...
        rankData:[
          {
            ...
            className:'tablePadding',
            align:'center',
            minWidth:this.$store.state.locale=='zh_CN'?30:60,
            ellipsis:true,
            key: 'prodUnitName'
          },{
            title: i18n.t("pqc.pp"),
            ...
            className:'tablePadding',
            width:this.$store.state.locale=='zh_CN'?86:130,
            minWidth:this.$store.state.locale=='zh_CN'?86:130,
            ellipsis:true,
            key: 'sampleQuantity'
          },{
            title: i18n.t("pqc.badNumber"),
            className:'tablePadding',
            width:this.$store.state.locale=='zh_CN'?66:120,
            minWidth:this.$store.state.locale=='zh_CN'?66:120,
            ellipsis:true,
            key: 'badQuantity'
          },{
            ...
            ellipsis:true,
            key: 'reviewState',
            // render:vm.$iqis.utils.tableHelper.buildMapCell(vm,{0:i18n.t('pqc.unaudited'),1:i18n.t('pqc.wr'),2:i18n.t('pqc.judged'),3:i18n.t('pqc.notClosed'),4:i18n.t('pqc.closed')})
            render:(h, params)=>{
              if(params.row.state===3||params.row.state===0){
                params.row.reviewState=null
              }
              let ret =  vm.$iqis.utils.tableHelper.buildMapCell(
                vm,{0:i18n.t('pqc.unaudited'),1:i18n.t('pqc.wr'),2:i18n.t('pqc.judged'),3:i18n.t('pqc.notClosed'),4:i18n.t('pqc.closed'),5:i18n.t('review')}
              )
              return ret(h,params);
            }
          },{
        ...
        table:{
          ...
          columns:[
            ...
            },
            {
              ...
              title: i18n.t("pqc.actualSampling"),
              className:'tablePadding',
              width:this.$store.state.locale=='zh_CN'?86:110,
              minWidth:this.$store.state.locale=='zh_CN'?86:110,
              ellipsis:true,
              key: 'sampleQuantity'
            },{
              title: i18n.t("pqc.badNumber"),
              className:'tablePadding',
              width:this.$store.state.locale=='zh_CN'?66:100,
              minWidth:this.$store.state.locale=='zh_CN'?66:100,
              ellipsis:true,
              key: 'badQuantity'
            },{
              title: i18n.t("pqc.defectRate"),
    ...
    methods:{
      copySelectRow(){
        ...
        this.$emit("open-tab","exam-pqc-copy-"+selectedRow[0].pqcId,i18n.t('product.copy')+"-"+selectedRow[0].productName,"page-exam-pqc-add",{pqcId:selectedRow[0].pqcId});
      },
      createAbnormal(){
        let selectedRow = this.$refs.pageTable.getSelection()
      },
      createAbnormal(){
        ...
        if(selectedRow[0].state==0||selectedRow[0].state==3){
          that.$Notice.warning({title:i18n.t("pqc.ecb")})
          return;
        }
        if(selectedRow[0].result==1){
          that.$Notice.warning({title:i18n.t("pqc.tir")})
          return;
        }
        that.$http.get("/examinecenter/pqc/abnormal",{page:1,limit:999999}).then(function (resp){
          for(let i in resp.data){
            if(resp.data[i].qcNo==selectedRow[0].pqcNo){
              that.$Notice.warning({title:i18n.t("pqc.aeh")})
              return;
            }
          }
          that.$emit("open-tab","exam-pqc-abnormal-add",i18n.t("pqc.ape"),"page-exam-pqc-abnormal-add",{qcNo:selectedRow[0].pqcNo});
        })
      },
      //点击跳转到输入的页码
      goElevatorPage(){
      ...
      AuditOk(){
        ...
        for(let i=0;i<AuditRwo.length;i++) {
          that.$http.post(url+AuditRwo[i].pqcId+"&state="+that.Audit.fieldName,that.headers).then((res)=>{
      ...
      addPqc(){
        this.$emit("open-tab","exam-pqc-add",i18n.t("pqc.act"),"page-exam-pqc-add",null);
      },
      editSelectRow(e){
         let selectedRow = this.$refs.pageTable.getSelection();
         if(!selectedRow || selectedRow.length != 1){
         ...
         this.$emit("open-tab","exam-pqc-update-"+selectedRow[0].pqcId,i18n.t("pqc.roc")+selectedRow[0].productName,"page-exam-pqc-add",{pqcId:selectedRow[0].pqcId});
        ...
        if(selection.length!=1){
         ...
         if(ss && ss.length > 0){
           ...
           for(let i=0;i<ss.length;i++){
             delArr.push(this.$http.delete("/examinecenter/pqc/pqc/"+ss[i].pqcId));
           }
      ...
      refreshData(){
        ...
        if(this.search.dates && this.search.dates[0]){
          ...
          params.query='startDate='+startDate+'&endDate='+endDate+'&result='+that.search.result+'&state='+that.search.state+'&field='+that.search.fieldName+'&keyword='+that.search.query+'&customerId='+customerId;
        }else{
          params.query='result='+that.search.result+'&state='+that.search.state+'&field='+that.search.fieldName+'&keyword='+that.search.query+'&customerId='+customerId;
        }
      ...
      /**双击打开 */
      onDBClick(obj,index){
         this.$emit("open-tab","exam-pqc-update-"+obj.pqcId,i18n.t("pqc.roc")+obj.productName,"page-exam-pqc-add",{pqcId:obj.pqcId});
      },
      //自定义排序
      rank(){
        let that=this;
