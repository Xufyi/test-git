  ...
  export default {
    ...
    methods:{
      copySelectRow(){
        ...
        this.$emit("open-tab","exam-pqc-copy-"+selectedRow[0].ipqcId,i18n.t('product.copy')+"-"+selectedRow[0].productName,"page-exam-pqc-add",{ipqcId:selectedRow[0].ipqcId});
      },
      //点击跳转到输入的页码
      goElevatorPage(){
        var evtObj;
      ...
      exportStatement (index){
        ...
        if(index == 2){
          ...
          params.ipqcId = reviewList[0].ipqcId
        }
      ...
      AuditOk(){
        ...
        for(let i=0;i<AuditRwo.length;i++) {
          that.$http.post(url+AuditRwo[i].ipqcId+"&state="+that.Audit.fieldName,that.headers).then((res)=>{
      ...
      addPqc(){
        this.$emit("open-tab","exam-pqc-new-inspectrecord-add",i18n.t("pqc.apr"),"page-exam-pqc-new-inspectrecord-add",null);
      },
      editSelectRow(e){
         let selectedRow = this.$refs.pageTable.getSelection();
         if(!selectedRow || selectedRow.length != 1){
         ...
         this.$emit("open-tab","exam-pqc-new-inspectrecord-add-"+selectedRow[0].ipqcId,i18n.t("pqc.mpr")+selectedRow[0].productName,"page-exam-pqc-new-inspectrecord-add",{ipqcId:selectedRow[0].ipqcId});
        ...
        if(selection.length!=1){
         ...
         if(ss && ss.length > 0){
           ...
           for(let i=0;i<ss.length;i++){
             delArr.push(this.$http.delete("/ipqc/"+ss[i].ipqcId));
           }
      ...
      refreshData(){
        ...
        this.$http.get("/ipqc",{params:params}).then(function (resp) {
      ...
      /**双击打开 */
      onDBClick(obj,index){
         this.$emit("open-tab","exam-pqc-new-inspectrecord-add-"+obj.ipqcId,i18n.t("pqc.mpr")+obj.productName,"page-exam-pqc-new-inspectrecord-add",{ipqcId:obj.ipqcId});
      },
      //自定义排序
      rank(){
        let that=this;
