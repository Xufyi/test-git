  ...
  <div class="iqis-page">
    ...
    <div class="iqis-page-tool-row">
      <div class="iqis-page-tool-primary" style="font-size:0;">
        <Button type="ghost" v-if="permsBtn.INSERT_DIRECT_RATE" icon="plus" @click="addIqc">{{$t("pqc.add")}}</Button>
        <Button style="margin:0 8px;" type="ghost" v-if="permsBtn.UPDATE_DIRECT_RATE" icon="edit" :disabled="tableEditDis" @click="editSelectRow">{{$t("pqc.modify")}}</Button>
        <Poptip
          confirm
          ...
          <Button type="ghost" v-if="permsBtn.DELETE_DIRECT_RATE" icon="trash-a" :disabled="!table.selected">{{$t("pqc.delete")}}</Button>
        </Poptip>
      </div>
      <div class="iqis-page-tool-second" style="font-size:0;">
        ...
        <Dropdown style="margin-left: 8px" v-if="permsBtn.EXPORT_DIRECT_RATE">
  ...
  export default {
    ...
    data: function () {
      ...
      return {
        ...
        table:{
          ...
          columns:[
            ...
            // },
            {
              ...
              title: i18n.t("pqc.qp"),
              className:'tablePadding',
              width:this.$store.state.locale=='zh_CN'?96:135,
              minWidth:this.$store.state.locale=='zh_CN'?96:135,
              ellipsis:true,
        ...
        search: {
          ...
          prodList:[
            {value:"",label:i18n.t("pqc.all")},
            {value:"1",label:i18n.t("pqc.completed")},
            {value:"0",label:i18n.t("pqc.ip")}
          ],
          data: {
            searchTypes: [
              {
        ...
        rankData:[
          {
            ...
            title: i18n.t("pqc.qp"),
            className:'tablePadding',
            width:this.$store.state.locale=='zh_CN'?96:150,
            minWidth:this.$store.state.locale=='zh_CN'?96:150,
            ellipsis:true,
            key: 'totalProdQuantity'
          },{
            title: i18n.t("pqc.badNumber"),
          ...
          },
          {
            title: i18n.t("pqc.ps"),
            className:'tablePadding',
            width:this.$store.state.locale=='zh_CN'?81:170,
            minWidth:this.$store.state.locale=='zh_CN'?81:170,
            ellipsis:true,
            key: 'productionState',
        ...
        permsBtn:{//控制每个权限对应按钮的禁用状态
          ...
          "SELECT_DIRECT_RATE":false,
          "UPDATE_DIRECT_RATE":false,
          "DELETE_DIRECT_RATE":false,
          "EXPORT_DIRECT_RATE":false
        },
    ...
    methods:{
      ...
      exportStatement (index){
        ...
        let params = {}; //搜索条件
        if(index == 1){
          url = '/examinecenter/pqc/directRate/export/list';
        ...
        if(index == 2){
          url = '/examinecenter/pqc/directRate/export/report'
          ...
          params.badRecordId = reviewList[0].directRateId
        }
      ...
      ,
      addIqc(){
        this.$emit("open-tab","exam-pqc-bad-update-1",i18n.t("pqc.appr"),"page-exam-pqc-bad-update",null);
      },
      onCClick(obj,index){
        //console.log(obj)
      ...
      editSelectRow(e){
        ...
        //
        this.$emit("open-tab","exam-pqc-bad-update-"+selectedRow[0].directRateId,i18n.t("pqc.mtptr")+selectedRow[0].productName,"page-exam-pqc-bad-update",{directRateId:selectedRow[0].directRateId,proID:2});
        ...
        if(ss && ss.length > 0){
          ...
          for(let i=0;i<ss.length;i++){
            delArr.push(this.$http.delete("/examinecenter/pqc/directRate/"+ss[i].directRateId));
          }
      ...
      refreshData(){
        ...
        if(this.search.dates && this.search.dates[0]){
          ...
          params.startDate = this.$iqis.utils.getDayStart(1,sdate);
          params.endDate = this.$iqis.utils.getDayEnd(edate);
          query="startDate%3D"+params.startDate+"%26endDate%3D"+params.endDate+"%26field%3D"+params.fieldName+"%26keyword%3D"+params.query+'%26customerId%3D'+params.customerId;
        }else {
          query="field%3D"+params.fieldName+"%26keyword%3D"+params.query+'%26customerId%3D'+params.customerId;
        }

        let url="/examinecenter/pqc/directRate?page="+params.page+"&limit="+params.limit+"&query="+query;
