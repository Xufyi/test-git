<template>
  <div class="iqis-page">
        ...
        <Row type="flex" :gutter="24">
          <i-col span="4">
            ...
            <div class="iqis-page-block-ctrl">
              <Select :placeholder="i18n.t('null')" v-model="isirData.manufacturerId" :disabled='inputDisbale||noPermsDis' filterable>
                <Option value=" ">{{$t('iqc.null')}}</Option>
                <Option v-for="item in prodBatch.dataTable" :value="item.manufacturerId" :key="item.manufacturerId">{{ item.manufacturerName }}</Option>
              </Select>
            </div>
            <div class="iqis-page-block-label">零件料號/Part number:</div>
            <Select :label="ruleForm.productNo" remote :remote-method="remoteMethod" :loading="loadingRemote" :placeholder="i18n.t('pleaseChoose')" v-model="isirData.productId" :disabled='inputDisbale||noPermsDis' filterable @on-change="changeProductId(isirData.productId)">
          ...
          <i-col span="4">
            ...
            <div class="iqis-page-block-ctrl">
              <i-input  v-model="isirData.isirNo" :disabled='inputDisbale||noPermsDis' style="width: 100%;"></i-input>
            </div>
          ...
          <i-col span="4">
            ...
            <div class="iqis-page-block-ctrl">
              <Date-picker type="datetime" format="yyyy-MM-dd HH:mm" v-model="isirData.createTime" :clearable='false' :disabled='inputDisbale||noPermsDis' style="width: 100%"></Date-picker>
            </div>
            <div class="iqis-page-block-label">樣品重量/Sample weight:</div>
            <div class="iqis-page-block-ctrl">
                <i-input  v-model="isirData.sampleWeight" style="width: 100%;"></i-input>
            </div>
          </i-col>
          <i-col span="4">
            <div class="iqis-page-block-label">检验员/Inspector</div>
            <div class="iqis-page-block-ctrl">
              <Select :placeholder="i18n.t('pleaseChoose')" v-model="isirData.inspectorId" :disabled='inputDisbale||noPermsDis' filterable>
            ...
            <div class="iqis-page-block-ctrl">
              <!-- <i-input  v-model="isirData.sampleQuantity" number :disabled='inputDisbale||noPermsDis' style="width: 100%;" @on-blur ="getAql(ruleForm.sampleQuantity,'select')"> -->
              <i-input  v-model="isirData.sampleQuantity" number :disabled='inputDisbale||noPermsDis' style="width: 100%;">
          ...
          <i-col span="4">
            ...
            <div class="iqis-page-block-ctrl">
              <i-input  v-model="isirData.version" style="width: 100%;"></i-input>
            </div>
          </i-col>
        </Row>
        <Row type="flex" :gutter="24">
          <i-col span="24">
                <table class="tableStyle" bgcolor="#dcdee2" cellspacing="1px">
                    <tr bgcolor="white">
                        ...
                        <td style="width:50%;" bgcolor="#f8f8f9">结论/Decision</td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.initialReasonList">
                                <Checkbox label="NEW_PART" >新零件/New part</Checkbox>
                            </CheckboxGroup>
                        </td>
                        <td>
                            <RadioGroup v-model="isirData.decision">
                                <Radio label="APPROVED" >通过/Approved</Radio>
                            </RadioGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.initialReasonList">
                                <Checkbox label="PROD_MODIFY" >產品更改/Product modifilcation</Checkbox>
                            </CheckboxGroup>
                        </td>
                        <td>
                            <RadioGroup v-model="isirData.decision">
                                <Radio label="APPROVED_CONDITIONS" >有條件通過/Approved with conditions</Radio>
                            </RadioGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.initialReasonList">
                                <Checkbox label="PRODUCTION_RELOCATION" >產地更改/Production relocation</Checkbox>
                            </CheckboxGroup>
                            </td>
                        <td>
                            <RadioGroup v-model="isirData.decision">
                                <Radio label="REJECTED" >拒收/Rejected</Radio>
                            </RadioGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.initialReasonList">
                                <Checkbox label="CHANGE_PROCESS" >生產過程更改/Change of production process</Checkbox>
                            </CheckboxGroup>
                            </td>
                        <td>
                            <RadioGroup v-model="isirData.decision">
                                <Radio label="REJECTED_INSTALL" >拒收，採取措施可能安裝/Rejected,installation possible with measures</Radio>
                            </RadioGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.initialReasonList">
                                <Checkbox label="STOP_PRODUCTION" >長期停產/Longer stoppage of production</Checkbox>
                            </CheckboxGroup>
                        </td>
                        <td>
                            <RadioGroup v-model="isirData.decision">
                                <Radio label="NEW_SAMPIES" >拒收，要求重新送樣/Rejected,new sampies required</Radio>
                            </RadioGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.initialReasonList">
                                <Checkbox label="NEW_SUB_SUPPLIER" >新的分包商/New sub-supplier</Checkbox>
                            </CheckboxGroup>
                            </td>
                        <td></td>
                    </tr>
    ...
    <div style="margin-top:20px">
        <Row type="flex" :gutter="24">
          <i-col span="24">
                <table class="tableStyle" bgcolor="#dcdee2" cellspacing="1px">
                    <tr bgcolor="white">
                        <td bgcolor="#f8f8f9">檢驗報告/inspection report</td>
                        <td  colspan="2" bgcolor="#f8f8f9">初樣檢驗報告/Inital sample inspection report</td>
                    </tr>
                    <tr bgcolor="white">
                      <td>
                        <CheckboxGroup v-model="isirData.inspectionReportList">
                          <Checkbox label="DIMENSION_INSPECTION">尺寸檢驗/Dimension inspection</Checkbox>
                        </CheckboxGroup>
                      </td>
                      <td colspan="2">
                          <RadioGroup v-model="isirData.initialInspectionReport">
                            <Radio label="SAMPLE_INSPECTION_REPORT">初樣檢查報告/Initial sample inspection report</Radio>
                          </RadioGroup>
                      </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.inspectionReportList">
                              <Checkbox label="FUNCTION_INSPECTION" >功能檢查/Function inspection</Checkbox>
                            </CheckboxGroup>
                        </td>
                        <td colspan="2">
                          <CheckboxGroup style="margin-left:20px" v-model="isirData.subInitalInspectionReportList">
                              <Checkbox label="APPROVED_CONDITIONS" >初樣檢查/Initial sample inspection</Checkbox>
                          </CheckboxGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                          <CheckboxGroup v-model="isirData.inspectionReportList">
                              <Checkbox label="MATERIAL_INSPECTION" >材料檢驗/Material inspection</Checkbox>
                          </CheckboxGroup>
                        </td>
                        <td colspan="2">
                          <CheckboxGroup style="margin-left:20px" v-model="isirData.subInitalInspectionReportList">
                              <Checkbox label="SUBSEQUENT_INSPECTION" >後續樣品檢查/Subsequent sample inspection</Checkbox>
                          </CheckboxGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                          <CheckboxGroup v-model="isirData.inspectionReportList">
                              <Checkbox label="RELIABILITY_INSPECTION">可靠性檢查/Reliability inspection</Checkbox>
                          </CheckboxGroup>
                        </td>
                        <td colspan="2">
                          <CheckboxGroup style="margin-left:20px" v-model="isirData.subInitalInspectionReportList">
                              <Checkbox label="PRE_SAMPLE_INSPECTION">預初樣檢驗/Pre sample inspection</Checkbox>
                          </CheckboxGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.inspectionReportList">
                                <Checkbox label="VISUAL_INSPECTION" >外觀檢查/Visual inspection</Checkbox>
                            </CheckboxGroup>
                        </td>
                        <td colspan="2">
                            <RadioGroup v-model="isirData.initialInspectionReport">
                                <Radio label="OTHER_INSPECTION_REPORT" >其他樣品檢查報告/Inspection report,other samples</Radio>
                            </RadioGroup>
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                          <CheckboxGroup v-model="isirData.inspectionReportList">
                              <Checkbox label="ATTRIBUTIVE_INSPECTION" >屬性檢查/Attributive inspection</Checkbox>
                          </CheckboxGroup>
                        </td>
                        <td style="width:25%">送樣次數/Sample no:</td>
                        <td style="width:25%" class="td_input">
                          <Input v-model="isirData.sampleNum" size="large" placeholder="请输入" />
                        </td>
                    </tr>
                    <tr bgcolor="white">
                        <td>
                            <CheckboxGroup v-model="isirData.inspectionReportList">
                                <Checkbox label="OTHERS" >其他/Others</Checkbox>
                            </CheckboxGroup>
                            </td>
                        <td colspan="2"></td>
                    </tr>
    ...
    <div style="padding-top:20px">
     ...
     <Row type="flex" :gutter="24">
        <i-col span="24">
          <Input v-model="isirData.remarks" type="textarea" placeholder="" />
        </i-col>
      </Row>
    </div>

    <!-- <div class="iqis-page-block-title">参数记录</div>
    <div class="iqis-page-block-table">
          <Table  :no-data-text="i18n.t('noData')" ref="paramTableFects" class="iqis-inline-editor-table noHeight defectTable noBorder location"  @on-selection-change="defectTableSelectChange"  border stripe
            ...
            <Button :disabled="noPermsDis" @click="addDefect" v-if="!inputDisbale" class="iqis-table-footer-btn" slot="footer" type="ghost" style="width:100%;margin: 16px;" icon="plus">添加参数记录</Button>
          </Table>
        </div> -->
    <!--参数记录-->
    <!-- <div class="iqis-page-block">
    ...
    <!-- <div class="iqis-page-block">
      ...
      <div class="iqis-page-block-body">
        <div class="iqis-page-block-tools">
          <div class="iqis-page-block-tools-primary">
            <div class="iqis-page-block-label">{{$t('iqc.attrTable')}}</div>
          </div>
          <div class="iqis-page-block-tools-second" style="margin-top: 6px;">
          </div>
        </div>
    ...
    <div class="iqis-page-block">
      ...
      <div class="iqis-page-block-body">
        <div class="iqis-page-block-tools">
          ...
          <div class="iqis-page-block-tools-second" style="margin-top: 6px;">
            <span class="iqis-page-block-table-tip" style="margin-right: 24px;line-height: 32px;">{{$t('iqc.changed')}} <span
              style="color:#1b75ea">{{prodBatch.TableLength}}</span> 项参数记录</span>
        ...
        <div class="iqis-page-block-table">
          <Table  :no-data-text="i18n.t('noData')" ref="paramTableFects" class="iqis-inline-editor-table noHeight defectTable noBorder location"  @on-selection-change="defectTableSelectChange"  border stripe
                  :loading="table.defect.loading" v-drag-table-column="table.defect.columns" :columns="table.defect.columns" :data="iqcDefects" >
            <Button disabled @click="addDefect" v-if="!inputDisbale" class="iqis-table-footer-btn" slot="footer" type="ghost" style="width:100%;margin: 16px;" icon="plus">添加参数记录</Button>
          </Table>
        </div>
    ...
    <!-- <div class="iqis-page-block" v-if="block.review">
      <div class="iqis-page-block-title">
        {{$t('iqc.treatmentRecord')}}
      </div>
      <div class="iqis-page-block-body">
        <div v-for="(items,index) in HandleList" :key="index">
          <div class="review-block">
            <Row type="flex" :gutter="24">
              ...
              <i-col span="4" style="width: 20% !important;">
                ...
                <div class="iqis-page-block-ctrl">
                  <DatePicker  format="yyyy-MM-dd HH:mm" type="datetime"  v-model="items.dataTime" :disabled="true" style="width: 100%"></DatePicker>
                </div>
              </i-col>
            </Row>
            <Row>
              <i-col span="24">
                <div v-if="index==0" class="iqis-page-block-label">{{$t('iqc.auditOpinion')}}</div>
                <div v-else-if="index<5" class="iqis-page-block-label">{{$t('iqc.reviewOpinion')}}</div>
                <div v-else class="iqis-page-block-label">{{$t('iqc.judgeOpinion')}}</div>
                <div class="iqis-page-block-ctrl">
        ...
        <Row type="flex" :gutter="24">
          <i-col span="24">
            ...
            <div class="iqis-page-block-ctrl">
              <i-input type="textarea" :autosize="{minRows: 1,maxRows: 6}" v-model="ruleForm.caseExplain" :disabled='this.reviewState!=3||this.ruleForm.inspectorId!=this.userids||this.noPermsDis' style="width: 100%;"></i-input>
            </div>
          </i-col>
        </Row>
      </div>
    </div>
    <div v-if="reviewState==4" style="text-align:center;color:#999;font-size:16px;font-weight: bold;">{{$t('iqc.endProcess')}}</div> -->
    ...
    <IqisAffix placement="bottom" :offset="0">
      <div class="iqis-page-save-tool" style="background-color: #fff; border-top: 1px solid #e8e8e8;width:100%;">
        <Button type="primary" v-if="reviewState==3&&userids == ruleForm.inspectorId" @click="endCase">{{$t('iqc.end')}}1</Button>
        <Button type="primary" @click="submData" :disabled="submit1" v-if="button.review&&permsBtn.UPDATE_IQC">{{$t('iqc.commit')}}2</Button>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        isirData:{
          initialReasonList:[],//送样原因
          decision:null,//结论
          inspectionReportList:[],
          initialInspectionReport:null,//检验报告
          subInitalInspectionReportList:[],//子初样检验报告
          sampleNum:null,
          remarks:null,
          isirNo:null,//檢查報告編號
          createTime:new Date(),
          ...
          manufacturerId:null,
          version:null,
          inspectorId:null,//检验员
          sampleWeight:null,//样品重量
          sampleQuantity:null,//送样数量
          isirParamRecordDtoList:[],//参数记录表格数据
          paramValueArray:[],//参数记录表格中"记录"的数据
        },
        chkDevListArr:[],
          ...
          param: {
            ...
            columns: [
              ...
              },
              {
                ...
                //   return h('Select', {
                //       props:{
                //           value: this.iqcParams[params.index].chkDevId,
                //           disabled:this.inputDisbale||this.noPermsDis,
                //           placeholder:i18n.t('Unlimited')
                //       },
                //       on: {
                //           'on-change':(event) => {
                //               this.iqcParams[params.index].chkDevId = event;
                //               // alert(this.iqcParams[params.index].chkDevId)
                //           }
                //       },
                //     },
              ...
              
              {
                ...
                render: (h, params) => {
                  ...
                  return h('Select',
                    {
                      props:{
                        value: this.chkDevListArr[params.index].chkDevId,
                    ...
                    this.chkDevListArr.map(function(type){
                      return h('Option', {
                        props: {value: type.chkDevId,label:type.name}
                      }, type);
                    })
    ...
    mounted () {
      let that = this;
      that.$http.get("/basic/chkdev",{params:{limit:99999999}}).then(function(res){
        console.log(res.data,"YYYYYYYYYYYYYYYYYYYYYYYY");
        that.chkDevListArr = res.data;
      })
      let isirId = that.$props.params && that.$props.params.isirId;
      if(isirId){
          that.$http.get("/isir/"+isirId).then((res) => {
    ...
    methods: {
      ...
      refreshData(iqcId) {
        ...
        that.$http.get('/basic/reviewprocess?page=1&limit=9999').then((res) => {
          ...
          that.$http.get(url + iqcId).then((res) => {
            ...
            if ((that.state == 2 || that.state == 1) && res.data.iqcItem.reviewProcessId != null) {//待审核或审核通过
              that.ReviewRecord(iqcId)
            }
      ...
      deleteTableSelected() {
        ...
        for (let i = 0; i < that.iqcDefects.length; i++) {
          for (let j = 0; j < ss.length; j++) {
            if (that.iqcDefects[i].defectNumber === ss[j].defectNumber) {
      ...
      submitBtn() {
        let that = this;
        let arr = JSON.parse(JSON.stringify(that.isirData))
        arr.createTime = new Date(arr.createTime).getTime()
        that.$http.post("/isir/commit",arr,that.headers).then((res) => {
          console.log(res,"IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII");
      ...
      save() {
        ...
        let arr = JSON.parse(JSON.stringify(that.isirData))
        arr.createTime = new Date(arr.createTime).getTime()
        that.$http.post("/isir/add",arr,that.headers).then((res) => {
          console.log(this.isirData,"this.isirData");
        })
        return;
        /* let params = [
          {type: 'Empty', title: i18n.t('iqc.productNo'), value: that.ruleForm.productId},
