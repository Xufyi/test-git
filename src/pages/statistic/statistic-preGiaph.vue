<template>
  <div class="iqis-page">
    ...
    <div class="iqis-page-context statistic-histogram-page1">
      <div id="search-bar" class="search-bar">
        <div class="form-inline" role="form">
          ...
          <div id="exam-report-date-pick" class="ivu-line" style='margin-top:10px;'>
            ...
            <Button style='float:right;' v-show="isChartShow == true" type="primary" icon="android-open" @click="onExportReport('exam-reportbzt')">{{$t('statistical.exportReport')}}</Button>
          </div>
      ...
      <div id="report-tab" class="tab-content exam-report-main">
        ...
        <div id="exam-reportbzt" class="exam-report-border" v-show="isChartShow">
          <!-- report title -->
          ...
          <div ref="histogramChart" id='histogramChartbzt' class="exam-report-chart-div"
          ...
          <div class="exam-report-datas-div">
            <table class="exam-report-datas-table">
              ...
              <tr class="report-data-thead">
                ...
                <td>{{$t('statistical.totalProportion')}}</td>
                ...
                <th>{{$t('statistical.inspectionTotal')}}</th>
                <th>{{$t('statistical.abnormalTotal')}}</th>
                <th>{{$t('statistical.totalProportion')}}</th>
              </tr>
              </thead>
  ...
  export default {
    data() {
      ...
      return {
        i18n:window.i18n,
        seriesDataValue:[],
        seriesDataValueSum:0,
        chartType: i18n.t('statistical.pie'),
    ...
    methods: {
      ...
      getReportData() {
        ...
        this.$http.get("/diagram/report/" + reportType + "/dispatcher", {params}).then(function (resp) {
            /* if(JSON.stringify(resp.data) == "{}" || resp.data.prodLineCount == null){ 
      ...
      showDataSelectModal(columns, data) {
        ...
        if(data){
         ...
         //允收率 批退率 不良率
          if((that.statisticIndicator == 'acceptance_rate' || that.statisticIndicator == 'return_rate' || that.statisticIndicator == 'bad_rate') && data.length){
            let overAllRate = 0;
            let arrayRate = [];
            let totalProportion = 0;
            for(let i = 0; i < data.length; i++){
              arrayRate.push(Number(data[i].rate))
            }
            //计算总允收率
            for (let i = 0; i < arrayRate.length; i++) {
              overAllRate+=arrayRate[i]
            }
            //计算总占比
            for (let i = 0; i < arrayRate.length; i++) {
              totalProportion = (arrayRate[i] / overAllRate) * 100;
              // totalProportion = numeral(totalProportion).format('0.00')
              totalProportion = Math.floor(totalProportion * 100) / 100
              data[i].totalProportion = totalProportion
            }
          }

           //直通率
          if(that.statisticIndicator == 'first_pass_yield' && data.length){
            let overAllFpy = 0;
            let arrayFpy = [];
            let totalProportion = 0;
            for(let i = 0; i < data.length; i++){
              arrayFpy.push(Number(data[i].fpy))
            }
            //计算总允收率
            for (let i = 0; i < arrayFpy.length; i++) {
              overAllFpy+=arrayFpy[i]
            }
            //计算总占比
            for (let i = 0; i < arrayFpy.length; i++) {
              totalProportion = (arrayFpy[i] / overAllFpy) * 100;
              totalProportion = numeral(totalProportion).format('0.00')
              data[i].totalProportion = totalProportion
            }
          }
          ...
          //客诉次数
          if(that.statisticIndicator == 'customer_complaint_count' && data.length){
            let overAllCount = 0;
            let arrayCount = [];
            let totalProportion = 0;
            for(let i = 0; i < data.length; i++){
              arrayCount.push(Number(data[i].count))
            }
            //计算总允收率
            for (let i = 0; i < arrayCount.length; i++) {
              overAllCount+=arrayCount[i]
            }
            //计算总占比
            for (let i = 0; i < arrayCount.length; i++) {
              totalProportion = (arrayCount[i] / overAllCount) * 100;
              totalProportion = numeral(totalProportion).format('0.00')
              data[i].totalProportion = totalProportion
            }
          }
      ...
      fillReportText(selectRows) {
        ...
        if(reportTitle=='客诉客诉数量饼状图'){
          reportTitle=='客诉数量饼状图'
        }
        if(this.$store.state.locale=='en_US'){
          
          reportTitle =this.buzNameMap[reportType]+ ' ' + rconfig.reportTitle+' ' +this.chartType 
          if(reportTitle=='Customer Complaint Customer Complaint Quantity Pie'){
            reportTitle='Customer Complaint Quantity Pie'
          }
        }
        ...
        var reportThead = $(".statistic-histogram-page1 .report-data-thead");
        var reportTbody = $(".statistic-histogram-page1 .report-datas-body");
        reportThead.empty();
        reportTbody.empty();

        var columnCount = 2; //双栏显示
        if (columnConfig) {
          $(".statistic-histogram-page1 .exam-report-datas-table").show();
          ...
          for (var i = 0; i < this.selectRows.length; i += columnCount) {
            var one = this.selectRows[i];
            var two = this.selectRows[i + 1];
            var rowIndex = Math.floor(i / 2);
            reportTbody.append(rowTemplate);
            var tr = $(reportTbody.children()[rowIndex]);
            for (var j = 0; j < clen; j++) {
              if (one) {
                $(tr.children()[j]).text(one[columnConfig[j].field]);
              }
              if (two && columnCount == 2) {
                $(tr.children()[j + clen + 1]).text(two[columnConfig[j].field]);
              }
            }
          }

        } else {
          $(".statistic-histogram-page1 .exam-report-datas-table").hide();
        }
      },
      updateHistogramChart(selectRows) {
        let that = this;
        if (!that.histogramChart) {
          return;
        }
        ...
        var rconfig = reportConfig[that.statisticIndicator] || {};
        var columnConfig = rconfig.columns;
        ...
        /* for(let i = 0; i <that.seriesDataValue.length; i++ ){
            // that.seriesDataValueSum+=(that.seriesDataValue[i])
            let num =  Number(that.seriesDataValue[i])
            that.seriesDataValueSum+=num

          } */
          // alert(that.seriesDataValueSum)
        if (minY == null) {
          minY = 0;
        } else {
        ...
        var option = {
              tooltip: {
                trigger: 'item',
                formatter: `i18n.t('statistical.name'):{b} <br/>i18n.t('statistical.totalProportion'): {d}%`
              },
              legend: {
                type: 'scroll',
                orient: 'vertical',
        ...
        let width = $('.statistic-histogram-page1').width();
...
<style>
  ...
  .statistic-histogram-page1 {
    margin-bottom: 10px;
    padding-bottom: 48px
  }

  .statistic-histogram-page1 .ivu-line {
    padding-bottom: 3px;
  }

  .statistic-histogram-page1 .exam-report-main {
    margin-top: 10px;
    /* margin-right: 10px; */
  ...
  .statistic-histogram-page1 .iqis-table-no-data {
    text-align: center;
    color: #333;
    font-size: 16px;
  }

  .statistic-histogram-page1 .exam-report-title-table,
  .statistic-histogram-page1 .exam-report-total-table,
  .statistic-histogram-page1 .exam-report-datas-table,
  .statistic-histogram-page1 .exam-report-footer {
    width: 100%;
    border: 0;
    border-collapse: collapse;
  }

  .statistic-histogram-page1 .exam-report-border {
    border: 1px solid #666;
    margin-bottom: 0;
  }

  .statistic-histogram-page1 .exam-report-total-border {
    border: 1px solid #666;
    margin-left: 18px;
    margin-right: 18px;
    padding: 6px 10px;
  ...
  .statistic-histogram-page1 .exam-total-item {
    margin-right: 30px;
  }

  .statistic-histogram-page1 .total-label {
    padding-right: 8px;
  }

  .statistic-histogram-page1 .exam-report-title-table .exam-report-date-cell {
    text-align: center;
  }

  .statistic-histogram-page1 .exam-report-title-table td {
    border: 0;
  ...
  .statistic-histogram-page1 .exam-report-title-table .exam-report-title-cell {
    font-size: 32px;
    line-height: 40px;
    text-align: center;
  }

  .statistic-histogram-page1 .exam-report-datas-div {
    width: 100%;
    padding: 0 12px;
    margin-top: 10px;
  }
  ...
  .statistic-histogram-page1 .exam-report-datas-table th,
  .statistic-histogram-page1 .exam-report-datas-table td {
  ...
  .statistic-histogram-page1 .exam-report-datas-table td.table-gap,
  .statistic-histogram-page1 .exam-report-datas-table th.table-gap {
    border-top: 0;
    border-bottom: 0;
    width: 10px;
    background-color: #fff;
